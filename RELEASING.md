...
1. Update the selected package version in `package.json`, both plugin manifests, CLI version, every skill's metadata, and its changelog. Other packages retain their versions.
...
4. In a clean checkout with npm publishing access, run `bun run release lp` or `bun run release hyperliquid`. The command validates and publishes only the selected package. Never print or commit registry credentials.
5. Independently verify `npm view <name>@<version> repository dist` and install that exact version into a clean consumer project. Check CLI version, catalog, validation, and the expected skills and references.
